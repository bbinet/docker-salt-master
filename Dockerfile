...
RUN apt-get update && \
    ...
    vim less net-tools procps lsb-release git \
...
# Install Salt 3006 onedir from Broadcom packages (relenv bundles Python 3.10)
...
RUN curl -fsSL https://packages.broadcom.com/artifactory/api/security/keypair/SaltProjectKey/public \
    ...
    echo "deb [signed-by=/etc/apt/keyrings/salt-archive-keyring.gpg] https://packages.broadcom.com/artifactory/saltproject-deb/ stable main" \
      > /etc/apt/sources.list.d/salt.list && \
    apt-get update && \
    apt-get install -yq salt-common=${SALT_VERSION} salt-master=${SALT_VERSION} salt-api=${SALT_VERSION} && \
    rm -rf /var/lib/apt/lists/*
...
# Allow older minions (e.g. 3003) to connect to this 3006 master
RUN mkdir -p /etc/salt/master.d && \
    echo "minimum_auth_version: 0" > /etc/salt/master.d/minimum_auth_version.conf
